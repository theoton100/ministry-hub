    ...
    <PublicLayout>
      <section className="pt-10 pb-6 border-b border-black/10" style={{backgroundColor: '#ffffff'}}>
        <div className="container">
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            ...
            <p className="text-black/40 text-sm">Digital resources to strengthen your faith. Pay once, download instantly.</p>
          </motion.div>
        </div>
      </section>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {books.map((book, i) => (
                ...
                >
                  <div className="bg-white border border-black/10 rounded overflow-hidden group">
                    {book.coverImageUrl ? (
                      <div className="aspect-[3/4] bg-white/5 overflow-hidden relative">
                        ...
                        />
                        {book.pdfFileKey && (
                          <div className="absolute top-3 right-3 bg-white/70 backdrop-blur-sm text-black text-xs font-semibold px-2.5 py-1 rounded flex items-center gap-1.5">
                      ...
                      <div className="aspect-[3/4] bg-white/5 flex items-center justify-center relative">
                        <span className="text-black/20 text-6xl font-extrabold">{book.title[0]}</span>
                        {book.pdfFileKey && (
                          <div className="absolute top-3 right-3 bg-white/70 backdrop-blur-sm text-black text-xs font-semibold px-2.5 py-1 rounded flex items-center gap-1.5">
                            <Download className="h-3 w-3" />
                            Digital Download
                    ...
                    <div className="p-5">
                      ...
                      {book.description && (
                        <p className="text-black/40 text-sm line-clamp-3 mb-4">{book.description}</p>
                      )}
                      <div className="flex items-center justify-between">
                        <span className="text-brand font-extrabold text-xl">{formatPrice(book.priceInCents)}</span>
                        <Button
                          onClick={() => setSelectedBook(book)}
                          className="bg-brand hover:bg-brand-hover text-black font-bold text-sm h-9 px-5 rounded-sm gap-1.5"
                        >
            ...
            <div className="text-center py-16">
              <h2 className="text-xl font-bold text-black mb-3">Books Coming Soon</h2>
      ...
      <Dialog open={!!selectedBook} onOpenChange={(open) => { if (!open) { setSelectedBook(null); setBuying(false); } }}>
        <DialogContent className="bg-white border-black/10 text-black max-w-md">
          <DialogHeader>
            <DialogTitle className="text-black text-lg font-bold">Purchase & Download</DialogTitle>
            <DialogDescription className="text-black/40 text-sm">
              {selectedBook?.title} — {selectedBook ? formatPrice(selectedBook.priceInCents) : ""}
            </DialogDescription>
          </DialogHeader>

          <div className="bg-white/5 border border-black/10 rounded p-3 flex items-start gap-3 mt-1">
            <Download className="h-5 w-5 text-brand shrink-0 mt-0.5" />
            <div>
              <p className="text-black/80 text-sm font-medium">Digital Download</p>
