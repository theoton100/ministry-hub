    ...
    <PublicLayout>
      <section className="pt-10 pb-6 border-b border-black/10">
        <div className="container">
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            <h1 className="text-4xl md:text-5xl font-extrabold text-black tracking-tight mb-2">Sermons</h1>
            <p className="text-black/40 text-sm">Watch our latest messages and let the Word transform your life.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
          {activeSermon ? (
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="max-w-4xl">
              <div className="aspect-video rounded overflow-hidden bg-white mb-6">
                <iframe
              ...
              <h2 className="text-2xl md:text-3xl font-extrabold text-black mb-2">{activeSermon.title}</h2>
              {activeSermon.description && (
                <p className="text-black/50 text-sm leading-relaxed max-w-2xl">{activeSermon.description}</p>
              )}
            ...
            <div className="text-center py-16">
              <h2 className="text-2xl font-extrabold text-black mb-2">Sermons Coming Soon</h2>
              <p className="text-black/40 text-sm">We are preparing powerful messages for you. Check back soon.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {sermons && sermons.length > 1 && (
        <section className="py-10">
          <div className="container">
            <h2 className="text-2xl font-extrabold text-black mb-8">Sermon Archive</h2>
              ...
              {sermons.map((sermon, i) => (
                <motion.div key={sermon.id} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUpDelay(i * 0.05)}>
                  ...
                  >
                    ...
                    <p className="text-black/30 text-xs font-semibold uppercase tracking-wider mb-1">
                      {new Date(sermon.publishedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </p>
                    <h3 className="text-base font-bold text-black group-hover:text-brand transition-colors line-clamp-2">{sermon.title}</h3>
                  </div>
                </motion.div>
              ))}
            </div>
