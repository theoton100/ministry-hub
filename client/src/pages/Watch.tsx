    ...
    <PublicLayout>
      <section className="bg-gradient-to-b from-foreground to-foreground/90 text-background py-20 md:py-28">
        <div className="container text-center">
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            <p className="text-primary font-medium tracking-widest uppercase text-sm mb-3">Watch</p>
      ...
      <section className="py-12 md:py-16 bg-background">
        <div className="container">
          {isLoading ? (
            <div className="max-w-4xl mx-auto"><Skeleton className="w-full aspect-video rounded-2xl" /><Skeleton className="h-8 w-2/3 mt-6" /></div>
          ) : activeSermon ? (
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="max-w-4xl mx-auto">
              ...
              <div className="mt-6">
                ...
                <p className="text-sm text-muted-foreground/60 mt-2">{new Date(activeSermon.publishedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
              </div>
            </motion.div>
          ) : (
            <div className="text-center py-20">
              <Play className="h-20 w-20 text-muted-foreground/20 mx-auto mb-6" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {sermons.map((sermon, i) => (
                <motion.div key={sermon.id} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUpDelay(i * 0.05)}>
                  <Card
                    className={`group overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl ${activeSermon?.id === sermon.id ? "ring-2 ring-primary shadow-lg" : "border-border/50"}`}
                  ...
                  >
                    <div className="relative aspect-video overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="w-14 h-14 rounded-full bg-white/90 flex items-center justify-center"><Play className="h-6 w-6 text-primary ml-0.5" /></div>
                      </div>
                    </div>
                    <CardContent className="p-4">
                      <h3 className="font-semibold text-sm line-clamp-2 group-hover:text-primary transition-colors">{sermon.title}</h3>
