      ...
      <section className="py-20 md:py-28 bg-yellow-400">
        <div className="container">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-3xl">
            <motion.p variants={fadeUp} className="text-gray-700 text-xs font-bold uppercase tracking-[0.3em] mb-4">
              Support the Mission
            </motion.p>
            <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl font-bold text-gray-900 leading-tight mb-6">
              Give & Make an Impact
            </motion.h1>
            <motion.p variants={fadeUp} className="text-gray-800 text-base leading-relaxed max-w-2xl">
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            <p className="text-gray-600 text-xs font-bold uppercase tracking-[0.3em] mb-4">Why Give?</p>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Your Gift Matters</h2>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {reasons.map((reason, i) => {
              ...
              return (
                <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUpDelay(i * 0.1)} className="bg-white border border-gray-200 rounded-lg p-8 hover:shadow-lg transition-all duration-300 group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 max-w-5xl">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-8 hover:border-gray-300 transition-colors">
                ...
                <p className="text-gray-600 font-bold text-sm">— 2 Corinthians 9:7</p>
              </div>
            </motion.div>

            {/* Right: Donation Form */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUpDelay(0.1)} className="lg:col-span-3">
              <form onSubmit={handleDonate} className="space-y-8 bg-white border border-gray-200 rounded-lg p-8 hover:border-gray-300 transition-colors">
                {/* Preset Amounts */}
                <div>
                  <label className="text-gray-700 text-xs font-bold block mb-4 uppercase tracking-wider">
                    Select an Amount
                  </label>
                      ...
                      <button
                        ...
                        className={`h-12 rounded-sm text-sm font-bold transition-all duration-300 ${
                          selectedAmount === amount
                            ...
                            : "bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900 border border-gray-300 hover:border-yellow-400"
                        }`}
                      >
                        {formatPrice(amount)}
                      </button>
                ...
                {/* Custom Amount */}
                <div>
                  <label className="text-gray-700 text-xs font-bold block mb-2 uppercase tracking-wider">
                    Or Enter a Custom Amount (₵)
                  </label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-base font-bold">₵</span>
                    <Input
                      type="number"
                      min="1"
                      step="0.01"
                      ...
                      placeholder="0.00"
                      className="border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 h-12 text-base rounded-sm pl-8"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <label className="text-gray-700 text-xs font-bold block uppercase tracking-wider">
                    Your Details
                  </label>
                  <Input
                    value={name}
                    ...
                    className="border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 h-12 text-base rounded-sm"
                  />
                  <Input
                    type="email"
                    value={email}
                ...
                <div>
                  <Button
                    ...
                    className="w-full bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold text-base h-12 rounded-sm gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
