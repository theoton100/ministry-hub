...
export default function Navbar() {
  ...
  return (
    <header
      ...
      style={{
        backgroundColor: scrolled ? 'rgba(10, 14, 39, 0.98)' : 'rgba(10, 14, 39, 0.85)',
        backdropFilter: 'blur(12px)',
        borderColor: scrolled ? 'rgba(255,140,66,0.15)' : 'rgba(255,255,255,0.05)',
    ...
    >
      <nav className="container flex items-center justify-between h-16">
        ...
        <div className="hidden lg:flex items-center gap-3 shrink-0">
          <Link href="/give">
            <Button className="bg-[#ff8c42] hover:bg-[#ff7a2a] text-[#0a0e27] font-semibold text-sm h-10 px-6 rounded-sm">
              Give
            </Button>
          </Link>
        </div>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-6 border-t border-white/10">
                <Link href="/give" onClick={() => setOpen(false)}>
                  <Button className="w-full bg-[#ff8c42] hover:bg-[#ff7a2a] text-[#0a0e27] font-semibold text-sm h-11 rounded-sm">
                    Give Now
                  </Button>
                </Link>
              </div>
